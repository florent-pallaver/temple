...
import com.temple.util.file.media.stream.NoVideoStreamException;
...
public class UnixStreamMetadataFactory extends AbstractCDIBean implements StreamMetadataFactory {
	...
	@Override
	public VideoMetadata getVideoMetadata(File mediaFile) throws NoVideoStreamException {
		final FFProbeMetadata metadata = this.getMetadata(mediaFile);
		if (this.isDebugLoggable()) {
			this.debug(metadata);
		}
		if (metadata.streams.video == null) {
			throw new NoVideoStreamException(mediaFile);
		}
	...
	private FFProbeMetadata getMetadata(File mediaFile) {
		...
		try {
			...
			if (termination == 0) {
				...
				pm.format.size = mediaFile.length();
				return pm;
			} else {
