...
namespace temple\web\mail\phpmailer {
    ...
    final class MailerImpl extends \temple\web\mail\Mailer {
        ...
        public function sendMail(\temple\web\mail\Mail $mail) {
            $email = $this->createDelegate();
            ...
            if($mail->replyToMail) {
                $email->addReplyTo($mail->replyToMail, $mail->replyToAlias);
            }
            
            $email->Subject = self::$subjectPrefix . $mail->subject;
            //convert HTML into a basic plain-text alternative body
            $content = $mail->content;
            if($mail->usingTemplate) {
                $tpl = file_get_contents(self::$templatePath);
                $content = str_replace(['{{title}}', '{{content}}'], [$mail->subject, $content], $tpl);
            }
            $email->msgHTML($content);
            
            if (!$email->send()) {
                throw new \temple\web\mail\MailerException($email->ErrorInfo);
            }
        }
