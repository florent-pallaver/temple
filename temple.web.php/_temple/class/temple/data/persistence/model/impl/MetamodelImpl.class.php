...
class MetamodelImpl implements \temple\data\persistence\model\Metamodel {
	...
	public function __construct(ReflectionClass $class) {
		...
		$this->mappings = [];
		$this->relations = [];
		$this->logger->debug('loading metamodel');
		$this->loadFrom($class);
	...
	private function loadFrom(ReflectionClass $class) {
		...
		if ($parent) {
			$this->logger->debug('loading from ' . $parent->getName());
		...
		foreach ($class->getProperties() as $p) {
			...
			if (!($p->isStatic() || array_key_exists($pn, $this->mappings))) {
				$this->logger->debug('Default descriptor for field ' . $pn);
